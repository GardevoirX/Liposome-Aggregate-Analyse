...
class Analyzer:
    def __init__(self, fileName, argsFileName):
        ...
        self.headAtomSelection = fix_not_selected_mol(
            self.top, get_atom_selection(self.args.selHeadAtom)
        )
        self.headAtomIdx = get_index_of_selected_atom(self.top, self.headAtomSelection)
        self.tailAtomSelection = fix_not_selected_mol(
            self.top, get_atom_selection(self.args.selTailAtom)
        )
        self.tailAtomIdx = get_index_of_selected_atom(self.top, self.tailAtomSelection)
        self.totalResNum = len(self.headAtomIdx)
        self.notSolventResNum = np.sum(~np.isnan(self.headAtomIdx))
        self.selectedRes = np.arange(self.totalResNum)[~np.isnan(self.headAtomIdx)]
        self.notSelectedRes = np.arange(self.totalResNum)[np.isnan(self.headAtomIdx)]
        self.excludedRes = np.array(
            [
                iRes
                for iRes, res in enumerate(self.top.residues)
                if res.resname in MOLEXCLUDED
            ]
        )
        self.headAtomIdx_noNan = np.array(self.headAtomIdx[self.selectedRes], dtype=int)
        ...
        self.tailAtomIdx_noNan = np.array(self.tailAtomIdx[self.selectedRes], dtype=int)
    ...
    def get_leaflet_location(
        self, leafletType, iFrame=None, start=None, end=None, traj=None
    ):
        ...
        if iFrame is not None:
            if not self.locationGot[iFrame]:
                self._get_leaflet_location(
                    leafletType,
                    [self.leafletCollection[iFrame]],
                    self.traj[iFrame : iFrame + 1],
                )
            ...
            raise ValueError(
                "You should input either a start end location and a trajectory or a frame index."
            )
            ...
            if self.__attribute_need_update(iFrame, start, self.locationGot):
                for idx in leafletCollection[iFrame]:
                    ...
                    if leafletType == "membrane":
                        leafletCollection[iFrame][idx].location = np.average(
                            traj[iFrame].positions[atomIdx][::, -1]
                        )
                        ...
                        radius = np.linalg.norm(
                            traj[iFrame].positions[atomIdx] - center, axis=1
                        )
    ...
    def get_aggregate_molecules(self):
        ...
        self.aggResIdx = [
            iRes
            for iRes, res in enumerate(self.top.top.residues)
            if res.name not in MOLEXCLUDED
        ]
    ...
    def find_unassigned_molecules(self, iFrame):
        ...
        self.unassignedMolIdx[iFrame] = np.array(self.unassignedMolIdx[iFrame])
    ...
    def assign_molecules(self, iFrame):
        ...
        if not self.assignedFrame[iFrame]:
            self._assign_molecules(self.unassignedMolIdx[iFrame], iFrame)
    ...
    def _assign_molecules(self, unassignedMolIdx, iFrame):
        ...
        if hasattr(self.leafletCollection[actualIdx][(0, 1)], "assigned"):
            return
        if len(self.leafletCollection[actualIdx].keys()) == 2 and hasattr(
            self.leafletCollection[actualIdx][(0, 1)], "leafletType"
        ):
    ...
    def _assign_based_on_leaflet_location(self, actualIdx, unassignedMolIdx):
        ...
        assert hasattr(
            self.leafletCollection[actualIdx][(0, 1)], "location"
        ) and hasattr(self.leafletCollection[actualIdx][(0, 2)], "location"), (
        ...
        if leafletType == "membrane":
            headAtomCoord = self.traj[actualIdx].positions[headAtomIdx][::, -1]
        elif leafletType == "vesicle":
            headAtomCoord = np.linalg.norm(
                self.traj[actualIdx].positions[headAtomIdx]
                - self.leafletCollection[actualIdx][(0, 1)].center,
                axis=1,
            )
        inLeaflet1 = abs(headAtomCoord - location0) < abs(headAtomCoord - location1)
        self.leafletCollection[actualIdx][(0, 1)].add_new_mol(
            unassignedMolIdx[inLeaflet1]
        )
        self.leafletCollection[actualIdx][(0, 2)].add_new_mol(
            unassignedMolIdx[~inLeaflet1]
        )
    ...
    def _assign_based_on_neighbour(self, actualIdx, unassignedMolIdx):
        ...
        assigned = np.concatenate(
            [
                self.leafletCollection[actualIdx][key].molIdx
        ...
        y = np.concatenate(
            [
                np.full(len(self.leafletCollection[actualIdx][key].molIdx), i)
        ...
        clf.fit(X, y)
        result = clf.predict(
            self.traj[actualIdx]._pos[
                np.array(self.headAtomIdx[unassignedMolIdx], dtype=int)
            ]
        )
        # update
        for i, key in enumerate(self.leafletCollection[actualIdx].keys()):
            self.leafletCollection[actualIdx][key].add_new_mol(
                unassignedMolIdx[result == i]
            )
    ...
    def calculate_flip_flop_times(self, selIdx: np.array, startFrame: int):
        ...
        for iFrame in track(range(startFrame, 20001)):
            center = np.average(
                [
                    self.leafletCollection[iFrame][(0, 1)].center,
                    self.leafletCollection[iFrame][(0, 2)].center,
                ],
                axis=0,
            )
            ...
            bufferZone = (
                min(location1, location2) + 0.1 * thickness,
    ...
    def calculate_correlation(
        ...
        for iFrame in track(range(startFrame, endFrame)):
            clusterCenters = select_center_based_on_kcenter(
                nClusters, self.traj[iFrame]._pos[self.tailAtomIdx_noNan]
            )
            ...
            df["labels"] = self._assign_molecule_to_cluster(
                positions, np.array(clusterCenters, dtype=float)
            )
    ...
    def remove_waste_leaflet(self, iFrame, idx):
        ...
        for iAgg, location in self.leafletCollection[iFrame]:
            if iAgg != idx[0]:
                newAgg[(iAgg, location)] = self.leafletCollection[iFrame][
                    (iAgg, location)
                ]
            elif iAgg == idx[0]:
                if location > idx[1]:
                    newAgg[(iAgg, location - 1)] = self.leafletCollection[iFrame][
                        (iAgg, location)
                    ]
    ...
    def reindex(self, leafletType):
        ...
        for iFrame in track(range(len(self.leafletCollection) - 1)):
            if leafletType == "membrane":
                ...
                if len(nextLeaflet1.intersection(currentLeaflet1)) < len(
                    nextLeaflet1.intersection(currentLeaflet2)
                ):
                    (
                        self.leafletCollection[iFrame + 1][(0, 1)],
                        self.leafletCollection[iFrame + 1][(0, 2)],
                    ) = (
                        self.leafletCollection[iFrame + 1][(0, 2)],
                        self.leafletCollection[iFrame + 1][(0, 1)],
                    )
    ...
    def _cal_corrcoef(self, iFrame: int, df: pd.DataFrame):
        ...
        dfGroupby = (
            df.groupby(["location", "labels"]).count().unstack(fill_value=0).stack()
        )
