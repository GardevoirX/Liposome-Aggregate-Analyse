...
class PBCRemover:
    def __init__(self, fileName, topFile, ndxFile, outputFileName):
        ...
        setEnv(4)
        assert fileName != outputFileName, (
            "Your output file name is the name of your input trajectory!"
        )
    ...
    def run(self):
        ...
        for iFrame in track(range(self.trajLen)):
            iRound = 0
            lastLoss = 0
            self.boxXYZ, (self.a, self.b, self.c), self.boxCenter = self._get_box_info(
                iFrame
            )
            allAtomPosition = torch.clone(
                torch.tensor(self.u.trajectory[iFrame].positions, dtype=float)
            )
            selectedAtomPosition = allAtomPosition[self.selectedAtomIdx]
            while iRound < MAXITER:
                l = loss(
                    selectedAtomPosition + bias,
                    self.boxXYZ,
                    self.boxCenter,
                    self.a,
                    self.b,
                ...
                scheduler.step()
                print(
                    f"Frame {iFrame}    loss: {l.detach().numpy():.2f}    bias: {bias}"
                )
            ...
            self._update_position(
                self._move_to_unit_cell(
                    allAtomPosition + bias, self.boxXYZ, self.a, self.b, self.c
                )
                .detach()
                .numpy()
    ...
    def _get_box_info(self, iFrame):
        ...
        boxXYZ = torch.tensor(
            [self.u.trajectory[iFrame].triclinic_dimensions[i, i] for i in range(3)]
        )
    ...
    class Loss(nn.Module):
        ...
        def forward(self, position, boxXYZ, boxCenter, a, b, c):
            ...
            position = PBCRemover._move_to_unit_cell(
                PBCRemover, position, boxXYZ, a, b, c
            )
            r = PBCRemover._get_position_vector(
                PBCRemover, position, boxCenter, boxXYZ, a, b, c
            )
