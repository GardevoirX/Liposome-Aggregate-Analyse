...
class LeafletAssigner:
    ...
    def __init__(
        ...
        self.allSets = (
            self._read_allSets(allSetsFile, self.start, self.stop, self.skip, trjSkip)
            if isinstance(self.allSetsFile, str)
            else self._generate_allSets(
                self.selRes, self.start, self.stop, self.skip, trjSkip
            )
        )
    ...
    def run(self):
        ...
        for iChunk in range(self.nChunk):
            ...
            self.leafIdx.append(self.assign_leaflet_index(assignment, start, stop))
    ...
    def generate_adjacency_matrix_of_atom_pairs(self, start, stop):
        ...
        for iFrame in range(start, stop):
            ...
            if (iFrame == start) or (
                len(self.allSets[iFrame]) != len(self.allSets[iFrame - 1])
            ):
            ...
            selectedPairs = np.concatenate(pairDistances) < self.distanceCutoff
            aggGraph.append(
                self._generate_contact_graph(molPairs[selectedPairs], nSelectedMol)
            )
    ...
    def get_normal_vector_of_every_molecular(self, start, stop):
        ...
        subTraj = np.array(
            [
                np.copy(self.trajToAnalysis[iFrame].positions)
                for iFrame in range(start, stop)
            ]
        )
        molecularOrientation, headPos = self._get_mol_orientation_CUDA(
            subTraj,
            self.head_resIdxMatrix,
            self.tail_resIdxMatrix,
            self.nHeadAtomPerMol,
            self.nTailAtomPerMol,
        )
        for iFrame in range(start, stop):
            # print('Computing the normal vector of Frame {}'.format(iFrame))
            normals = self._calculate_normal_vector_from_cloud_point(
                headPos[iFrame - start][self.selRes]
            )
            toConvert = np.argwhere(
                (
                    self._product(
                        molecularOrientation[iFrame - start][self.selRes], normals
                    )
                )
                < 0
            )
    ...
    def _calculate_normal_vector_from_cloud_point(self, position):
        ...
        pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(
                radius=self.distanceCutoff, max_nn=40
            )
        )
    ...
    def assign_leaflet(self, aggGraph, normalVector):
        ...
        for iFrame in range(len(aggGraph)):
            ...
            productOfVector1 = self._product(
            ...
            pairsNotParallel = (
                productOfVector1 < parallelDegree
            )  # & (productOfVector2 < parallelDegree)
            ...
            for leaflet in assignment[iFrame - start]:
                for iAgg in range(len(self.allSets[iFrame])):
                    if leaflet[0] in self.allSets[iFrame][iAgg]:
                        leafletIdx[iFrame - start][(iAgg, leafletsInAgg[iAgg])] = (
                            Leaflet(iFrame, iAgg, leafletsInAgg[iAgg], leaflet)
                        )
                        leafletIdx[iFrame - start][
                            (iAgg, leafletsInAgg[iAgg])
                        ].get_composition(self.molType)
    ...
    def _prepare_info(self, top, selHeadAtom, selTailAtom):
        ...
        self.headAtomIdx = np.array(
            get_index_of_selected_atom(top, selHeadAtom), dtype=int
        )
        head_resIdxMatrix, nHeadAtomPerMol = self._get_atom_residue_matrix(
            top, selHeadAtom, self.molType
        )
        tail_resIdxMatrix, nTailAtomPerMol = self._get_atom_residue_matrix(
            top, selTailAtom, self.molType
        )
    ...
    def _get_mol_type(top):
        ...
        return np.array(
            [mol.resname for mol in top.residues if mol.resname not in RESEXCLUDED]
        )
                ...
                np.isin(
                    ...
                    np.array(
                        [
                            ...
                            if atom.name in selAtom[res.resname]
                        ]
                    ),
                )
                for res in traj.residues
                if res.resname not in RESEXCLUDED
            ]
        )
        nAtomPerMol = []
    ...
    def _get_mol_orientation_CUDA(
        ...
        molecularOrientation /= cp.expand_dims(
            cp.linalg.norm(molecularOrientation, axis=2), 2
        )
    ...
    def _write_results(self):
        ...
        args.selTailAtom = self.selTailAtomCmd
        args.stop = self.stop
