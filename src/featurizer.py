...
class Featurizer:
    ...
    def plot_polar_distribution(self, iFrame, selIdx, returnCoord=False):
        ...
        if len(selIdx.shape) > 1:
            selIdx = np.concatenate(selIdx)
        pos = np.copy(self.u.trajectory[iFrame].positions)
        com = np.average(pos, axis=0)
        pos -= com
        r, theta, phi = self._cart_to_sph(
            pos[selIdx, 0], pos[selIdx, 1], pos[selIdx, 2]
        )
        if returnCoord:
            return r, theta, phi
        ...
        sns.histplot(
            ...
            y=mirror[::, 1],
            ax=ax,
            weights=1 / np.abs(np.sin(mirror[::, 0])),
            bins=(50, 50),
        )
    ...
    def plot_radius_distribution(
        self, molName: str, iFrame: int, returnDF=False, **kwargs
    ):
        """
        ...
        atomName = set(
            [atom.name for atom in self.u.select_atoms(f"resname {molName}")]
        )
    ...
    def plot_mass_center_radius_distribution(
        self, iFrame: int, nMolCutoff: int = 0, noPlot=False
    ):
    ...
    def compute_asphericity_parameter(self, comFile):
        ...
        for iFrame in track(range(self.trjLen)):
            ...
            ap = ((l[0] - l[1]) ** 2 + (l[1] - l[2]) ** 2 + (l[2] - l[0]) ** 2) / (
                2 * (l[0] + l[1] + l[2]) ** 2
            )
    ...
    def calculate_contact_matrix(self, iFrame, selIdx):
        ...
        return distances.contact_matrix(frame.positions[selIdx], returntype="sparse")
