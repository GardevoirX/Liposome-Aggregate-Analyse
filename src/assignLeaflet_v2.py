...
from src.toolkit import setEnv, write_file
...
def main():
    ...
    nFrames = len(allSets)
    print(f"All sets read! Totally {nFrames} frames!")

    nChunk = int(nFrames / chunkSize)
    leafIdx = []
    for iChunk, chunk in track(
        enumerate(md.iterload(trajFile, chunk=chunkSize, top=top, skip=start)),
        total=nChunk,
    ):
        if iChunk > nChunk:
            break
        else:
            allSetsStart, allSetsStop = get_start_and_stop(
                iChunk, nChunk, chunkSize, nFrames
            )
        aggGraph = generate_adjacency_matrix_of_atom_pairs(
            chunk, headAtomIdx, allSets[allSetsStart:allSetsStop], distanceCutoff
        )
        ...
        leafIdx.append(
            assign_leaflet_index(assignment, allSets[allSetsStart:allSetsStop], molType)
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="To assign lipid to leaflets for every frames provided"
    )
    parser.add_argument(
        "-top",
        dest="topFile",
        help="All the format can be read by mdtraj is supported.",
    )
    ...
    parser.add_argument(
        "-set",
        dest="allSetsFile",
        help="The length of the all_sets file must be the same as the trjSkip times traj file's. If not provided, every molecule will be considered as in a single aggregate/",
        default=None,
    )
    parser.add_argument(
        "-head",
        dest="selHeadAtom",
        help="The atoms/beads chosen for every kind of molecules should be specified. For example, \"-head 'DPPC:NC3 PO4|DPPE:NH3 PO4'\".",
    )
    parser.add_argument("-tail", dest="selTailAtom")
    parser.add_argument(
        "-start",
        dest="start",
        help="Frames in traj[start:stop:skip] will be processed. If not provided, the whole trajectory will be processed.",
        default=None,
    )
    ...
    parser.add_argument(
        "-dist",
        dest="distanceCutoff",
        help="Only atoms within this cutoff will be considered for atom pairs and normal vector calculation.",
        default=1.66,
        type=float,
    )
    parser.add_argument(
        "-degree",
        dest="parallelDegreeCutoff",
        help="If the angle of two vectors is smaller than this value, the two vectors will be considered as a pair of parallel vectors.",
        default=14,
        type=float,
    )
    parser.add_argument(
        "-min",
        dest="minLeafletSize",
        help="Only the leaflet containing moleculars more than this value will be considered as a leaflet.",
        default=30,
        type=float,
    )
    parser.add_argument(
        "-chunk",
        dest="chunkSize",
        help="Parameter for mdtraj.iterload()",
        default=1000,
        type=int,
    )
    parser.add_argument(
        "-o", dest="outputPref", help="The file name of the output file.", default="./"
    )
    ...
    return (
        ...
        identify_num(args.start),
        identify_num(args.stop),
        args.skip,
        args.distanceCutoff,
        args.parallelDegreeCutoff,
        args.minLeafletSize,
        args.chunkSize,
        args.outputPref,
    )
...
def init(top, selHeadAtom, selTailAtom):
    ...
    head_resIdxMatrix, nHeadAtomPerMol = get_atom_residue_matrix(
        top, selHeadAtom, molType
    )
    tail_resIdxMatrix, nTailAtomPerMol = get_atom_residue_matrix(
        top, selTailAtom, molType
    )
    ...
    return (
        molType,
        headAtomIdx,
        cp.array(head_resIdxMatrix),
        cp.array(nHeadAtomPerMol),
        cp.array(tail_resIdxMatrix),
        cp.array(nTailAtomPerMol),
        selRes,
    )
...
def generate_adjacency_matrix_of_atom_pairs(trajectory, headAtomIdx, allSets, cutoff):
    ...
    for iFrame in range(trajLen):
        ...
        if (iFrame != 0) and (len(allSets[iFrame]) != len(allSets[iFrame - 1])):
            ...
            distances = (
                md.compute_distances(trajectory[lFrame:iFrame], atomPairs) < cutoff
            )
        ...
        print(f"Computing the distance matrix of Frame {lFrame} - {iFrame}")
        distances = (
            md.compute_distances(trajectory[lFrame : iFrame + 1], atomPairs) < cutoff
        )
        print(f"Constructing the adjacency matrix of Frame {lFrame} - {iFrame}")
        for distance in distances:
            G = nx.Graph()
...
def get_normal_vector_of_every_molecular(
    trajectory,
    selRes,
    head_resIdxMatrix,
    nHeadAtomPerMol,
    ...
    normalVector = {}
    molecularOrientation, headPos = get_mol_orientation_CUDA(
        trajectory.xyz,
        head_resIdxMatrix,
        tail_resIdxMatrix,
        nHeadAtomPerMol,
    ...
    for iFrame in range(len(trajectory)):
        print(f"Computing the normal vector of Frame {iFrame}")
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(headPos[iFrame])
        pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=cutoff, max_nn=20)
        )
...
def get_mol_orientation(
    xyz, head_resIdxMatrix, tail_resIdxMatrix, nHeadAtomPerMol, nTailAtomPerMol
):
...
def assign_leaflet(aggGraph, normalVector, degreeCutoff, minLeafletSize):
    ...
    for iFrame in range(len(aggGraph)):
        ...
        productOfVector = product(
            normalVector[iFrame][pairs[::, 0]], normalVector[iFrame][pairs[::, 1]]
        )
...
def read_allSets(fileName, start, stop, skip, trjSkip):
    ...
    allSets = list([allSets[key] for key in allSets])[start : stop : skip * trjSkip]
        ...
        for leaflet in assignment[iFrame]:
            for iAgg in range(len(allSets[iFrame])):
                if leaflet[0] in allSets[iFrame][iAgg]:
                    leafletIdx[iFrame][(iAgg, leafletsInAgg[iAgg])] = Leaflet(
                        iFrame, iAgg, leafletsInAgg[iAgg], leaflet
                    )
                    leafletIdx[iFrame][(iAgg, leafletsInAgg[iAgg])].get_composition(
                        molType
                    )
    ...
    atom_resIdxMatrix = np.array(
        [
            np.isin(
                ...
                np.array(
                    [
                        traj.top.residue(iRes).atom(atomName).index
